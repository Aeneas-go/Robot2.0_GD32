...
void MoveServo(uint8_t servoID, int16_t Angle, uint16_t Time)
{
	...
	usart_send_array(RobotTxBuf, 10);
}
...
void MoveServosByArray(RobotServo servos[], uint8_t Num, uint16_t Time)
{
	...
	usart_send_array(RobotTxBuf, RobotTxBuf[2] + 2);             //发送
}
/**
* @brief  控制舵机运行指定动作组
...
{
	RobotTxBuf[0]=RobotTxBuf[1]=FRAME_HEADER;
	RobotTxBuf[2]=2;
	RobotTxBuf[3]=CMD_GET_BATTERY_VOLTAGE;
	
	usart_send_array(RobotTxBuf,4);
}